...
def main(argv):
    ...
    if np.count_nonzero(connect) < data.shape[0]:
    ...
    print(f'Avg={avg}, std={std}, n_bin={n_bins}, bin_size={dx}, r_max={rmax}, r_NN_max={radius}\n')
    ...
    if Dfit > 25:
        print('NOTE: Dfit > 25 could indicate that the choice of the input parameters is not optimal or that the the distance distribution itself has some issue \n')
    ...
    statistics = str('# Npoints, rmax, standard deviation, R, D_fit, Dmin \n# \
     {}, {}, {}, {}, {}, {}\n'.format(np.count_nonzero(connect), rmax, std, ratio, Dfit, Dmin))
    ...
    if args.projection:
        ...
        np.savetxt('proj_' + str(input_f.split('.')[0]) + '.dat', proj[:, 0:Dmin + 1])
